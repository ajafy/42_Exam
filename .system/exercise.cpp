...
exercise *randomize_exercise(std::map<int, exercise> list, bool remove_success, int my_ex)
{
    ...
    if (my_ex >= list.size())
    {
        std::cout << "myError: all exercises for this level have been done" << std::endl;
        // std::cout << "myPlease set to" << RED << BOLD << " OFF " << RESET << "option 1 in settings." << std::endl;
        // std::cout << "myOr edit/delete your success/success_ex file" << std::endl;
        // std::cout << "myhen relaunch 42_EXAM and recover your exam" << std::endl;
        exit(0);
    }
    for (int i = 0; i < my_ex; i++)
        it++;
    return (&it->second);
}
